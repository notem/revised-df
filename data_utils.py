...
def load_data(directory, input_size, separator='\t', 
        fname_pattern=r"(\d+)[-](\d+)", max_classes=99999, 
        min_instances=0, max_instances=500, open_world=False):
    """
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for fname in tqdm(files, total=len(files)):
            ...
            with open(os.path.join(root, fname), "r") as fi:
                trace = load_trace(fi, separator, filter_by_size=True)
                ...
                if len(rep) < input_size:
                    rep = np.hstack((rep, np.zeros((input_size - len(rep),))))
                rep.resize((input_size, 1))
