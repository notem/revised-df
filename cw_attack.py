...
def train_model(X_tr, y_tr, X_va, y_va, classes, input_size, filepath):
    """
    """
    try:
...
def main():
    ...
    X, y, = load_data(args.traces, input_size=args.length, max_instances=100)
    size = len(y)
    ...
    # # # # # # # # 
    model = train_model(X_tr, y_tr, X_va, y_va, NB_CLASSES, args.length, args.output)
